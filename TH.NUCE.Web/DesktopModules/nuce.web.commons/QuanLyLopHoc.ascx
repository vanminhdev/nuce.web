<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="QuanLyLopHoc.ascx.cs" Inherits="nuce.web.commons.QuanLyLopHoc" %>
...
<script>
    ...
    $("#slKhoa").change(function () {
        //alert($('#slKhoa').val());
        quanlylophoc.initBoMon($('#slKhoa').val());
    });
    $("#slBoMon").change(function () {
        //alert($('#slKhoa').val());
        quanlylophoc.initMonHoc($('#slBoMon').val());
    });
    ...
    var quanlylophoc = {
        ...
        m_monhocid: -1,
        initMonHoc: function (bomonid) {
            // Chen nguoi dung
            $('#slGiangVien').html("");
            $.getJSON(this.url + "GetNguoiDungByBoMon.ashx?role=7&bomonid=" + bomonid, function (data) {
                //alert(data);
                $.each(data, function (i, item) {
                    $('#slGiangVien').append($('<option>', {
                        value: item.UserID,
            ...
            $('#slMonHoc').html("");
            $.getJSON(this.url + "GetMonHocNameByBoMon.ashx?bomonid=" + bomonid, function (data) {
                //alert(data);
                $.each(data, function (i, item) {
        ...
        initBoMon: function (khoaid) {
            ...
            $.getJSON(this.url + "GetBoMonByKhoa.ashx?khoaid=" + khoaid, function (data) {
                //alert(data);
                $.each(data, function (i, item) {
                    if (i == 0) { quanlylophoc.initMonHoc(item.BoMonID); }
        ...
        init: function () {
            ...
            $.getJSON(this.url + "GetKhoa.ashx", function (data) {
                //alert(data);
                $.each(data, function (i, item) {
                    if (i == 0) { quanlylophoc.initBoMon(item.KhoaID); }
        ...
        capnhat: function () {
            ...
            if ($("#txtMa").val().trim() == "" || $("#txtTen").val().trim() == "") {
                ...
                var url = quanlylophoc.url + "InsertLopHoc.ashx?" + str;
//                alert(str);
                if (strType == "update") {
                    url = quanlylophoc.url + "UpdateLopHoc.ashx?" + str;
                }
                $.get(url, function (data) {
                    if (data = "1") {
