...
<div class="row" style="padding-top: 5px;">
    ...
    <div class="col-sm-3">
        <div class="form-group">
            <select class="form-control" id="slChonNgay" onchange="QuanLyDichVu_GiayGioiThieu.changeNgayLayDuLieu(this);">
    ...
    <div class="col-sm-4" style="text-align: right">
        <button type="button" class="btn btn-success" 
                onclick="QuanLyDichVu_GiayGioiThieu.onClickConfirm(3)">Excel</button>
        <button type="button" class="btn btn-primary" 
                onclick="QuanLyDichVu_GiayGioiThieu.onClickConfirm(1)">In</button>
        <button type="button" class="btn btn-info" 
                onclick="QuanLyDichVu_GiayGioiThieu.onClickConfirm(2)">Chuyển trạng thái</button>
    </div>
</div>
<div>
            ...
            <tr>
                <th style="text-align: center; vertical-align: top;">
                    <input type="checkbox" class="checkbox-select"
                            id="select-all" onclick="QuanLyDichVu_GiayGioiThieu.onSelectAll()" />
                </th>
...
<script>
    ...
    var QuanLyDichVu_GiayGioiThieu = {
        ...
        afterConfirm: function(type) {
            ...
            const data = [];
            QuanLyDichVu_GiayGioiThieu.Data.forEach((item, index) => {
                if (QuanLyDichVu_GiayGioiThieu.selectedDataIndex.includes(index)) {
                    data.push({
                        ID: item.ID,
            ...
            switch(type) {
                    case 1:
                        ...
                        break;
                    case 2:
                        QuanLyDichVu_GiayGioiThieu.updateFourthStatus(jsonData);
                        break;
                    case 3:
        ...
        onClickConfirm: function(type) {
            if (QuanLyDichVu_GiayGioiThieu.selectedDataIndex.length > 0) {
                let content = '';
                switch(type) {
                    case 1:
                ...
                $('#btn-activate').click(function() { QuanLyDichVu_GiayGioiThieu.afterConfirm(type); });
        ...
        bindData: function () {
            $('#tbContent').html("");
            $('#ulPaging').html("");
            var SPage = (QuanLyDichVu_GiayGioiThieu.Page - 1) * QuanLyDichVu_GiayGioiThieu.RowCount + 1;
            var EPage = QuanLyDichVu_GiayGioiThieu.Page * QuanLyDichVu_GiayGioiThieu.RowCount;
            //Lấy dữ liệu
            $.getJSON(QuanLyDichVu_GiayGioiThieu.url + "ad_ctsv_qldv_xacnhan_get.ashx?Type=" + QuanLyDichVu_GiayGioiThieu.Type + "&&search=" + $('#myInput').val() + "&&CDay=" + $('#slChonNgay').val() + "&&SPage="+SPage+"&&EPage="+EPage, function (data) {
                //alert(data);
                $(`#select-all`).prop({checked: false, indeterminate: false});
                QuanLyDichVu_GiayGioiThieu.Data = data;
                QuanLyDichVu_GiayGioiThieu.SameStudentRequest = {};
                QuanLyDichVu_GiayGioiThieu.selectedDataIndex = [];
                var strHtml = "";
                var total = -1;
                $.each(data, function (i, item) {
                    total = item.Total;
                    if (item.StudentID in QuanLyDichVu_GiayGioiThieu.SameStudentRequest) {
                        QuanLyDichVu_GiayGioiThieu.SameStudentRequest[item.StudentID].push(i);
                    } else {
                        QuanLyDichVu_GiayGioiThieu.SameStudentRequest[item.StudentID] = [i];
                    }
                    ...
                    strHtml += `<input type="checkbox" id="${i}-select" 
                                    ...
                                    onclick="QuanLyDichVu_GiayGioiThieu.updateselectedDataIndex(${i}, 'select')">
                                </td>`;
                    ...
                    strHtml += "<td rowspan=\"2\" style=\"text-align: center; vertical-align: middle;\">";
                    strHtml += `<div class="vertical-group-btn"><button type="button" style="display: block; width: 100%" class="btn btn-primary"  onclick="QuanLyDichVu_GiayGioiThieu.print(${item.ID});">In</button>`;
                    strHtml += `<button type="button" style="display: block; width: 100%" class="btn btn-info" data-toggle="modal" data-target="#myModal" onclick="QuanLyDichVu_GiayGioiThieu.fillData(${item.ID});">Chuyển trạng thái</button>`;
                    strHtml += `</td>`;
                    strHtml += `</div>`;
                    strHtml += "</tr>";
                    // row 2
                    ...
                    strHtml += `
                        ...
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Pho" onkeyup="QuanLyDichVu_GiayGioiThieu.setValue(${i}, 'HKTT_Pho')" value="${item.HKTT_Pho || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Phường</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Phuong" onkeyup="QuanLyDichVu_GiayGioiThieu.setValue(${i}, 'HKTT_Phuong')" value="${item.HKTT_Phuong || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Quận/Huyện</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Quan" onkeyup="QuanLyDichVu_GiayGioiThieu.setValue(${i}, 'HKTT_Quan')" value="${item.HKTT_Quan || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Tỉnh/Thành phố</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Tinh" onkeyup="QuanLyDichVu_GiayGioiThieu.setValue(${i}, 'HKTT_Tinh')" value="${item.HKTT_Tinh || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="row">`;
                ...
                //Phân trang
                if (total > QuanLyDichVu_GiayGioiThieu.RowCount) {
                    // <li><a href="javascript:alert('hi hi');">1</a></li>
                    //class="active"
                    var strHtmlPage = "";
                    var numPage = 1 + Math.floor((total - 1) / QuanLyDichVu_GiayGioiThieu.RowCount);
                  
                    for (var i = 1; i <= numPage; i++) {
                        if (i != QuanLyDichVu_GiayGioiThieu.Page)
                            strHtmlPage += "<li><a href=\"javascript:QuanLyDichVu_GiayGioiThieu.changePage(" + i + ");\">" + i + "</a></li>";
                        else {
        ...
        print: function (input) {
            ...
            $('#myModalConfirm').modal('hide');
            let data = {};
            const selected = QuanLyDichVu_GiayGioiThieu.Data.find(item => item.ID === input);
        ...
        changeNgayLayDuLieu: function (obj) {
            ...
            QuanLyDichVu_GiayGioiThieu.Page = 1;
            QuanLyDichVu_GiayGioiThieu.bindData();
        },
        changePage: function (page) {
            //alert($("#slChonNgay").val());
            QuanLyDichVu_GiayGioiThieu.Page = page;
            QuanLyDichVu_GiayGioiThieu.bindData();
        }
    };
