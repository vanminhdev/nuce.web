<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="QuanLyDichVu_MuonHocBaGoc.ascx.cs" Inherits="nuce.ad.ctsv.QuanLyDichVu_MuonHocBaGoc" %>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <select class="form-control" id="TrangThai" onchange="QuanLyDichVu_MuonHocBaGoc.changeTrangThai();">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-default" id="btnCapNhat" onclick=" QuanLyDichVu_MuonHocBaGoc.update();">Cập nhật</button>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="text-align: center;">
                ...
                <button type="button" class="btn btn-danger" id="btnXoa" onclick=" QuanLyDichVu_MuonHocBaGoc.delete();">Thực hiện</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    var QuanLyDichVu_MuonHocBaGoc = {
        ...
        bindData: function () {
            $('#tbContent').html("");
            $.getJSON(this.url + "ad_ctsv_qldv_xacnhan_get.ashx?Type=" + QuanLyDichVu_MuonHocBaGoc.Type+ "&&search=" + $('#myInput').val(), function (data) {
                //alert(data);
                QuanLyDichVu_MuonHocBaGoc.Data = data;
                var strHtml = "";
                $.each(data, function (i, item) {
                    ...
                    if (item.Status < 4 && item.Status > 1) {
                        strHtml += "<td style=\"text-align: center; vertical-align: top;\"><button type=\"button\" class=\"btn btn-info\" data-toggle=\"modal\" data-target=\"#myModal\" onclick=\"QuanLyDichVu_MuonHocBaGoc.fillData(" + item.ID + ");\">Chuyển trạng thái</button></td>";
                    }
                    else {
                        strHtml += "<td></td>";
                    }
                    strHtml += "<td style=\"text-align: center; vertical-align: top;\"><button type=\"button\" class=\"btn btn-primary\"  onclick=\"QuanLyDichVu_MuonHocBaGoc.print('" + item.ID + "');\">In</button></td>";
                    strHtml += "</tr>";
        ...
        fillData: function (id) {
            QuanLyDichVu_MuonHocBaGoc.ID = id;
            $("#edit_header").html("Chuyển trạng thái");
            $("#edit_header_ThongBao").hide();
            $.each(QuanLyDichVu_MuonHocBaGoc.Data, function (i, item) {
                if (item.ID == id) {
                    ...
                    QuanLyDichVu_MuonHocBaGoc.showHideFormTrangThai(item.Status);
                }
        ...
        {
            QuanLyDichVu_MuonHocBaGoc.showHideFormTrangThai($("#TrangThai").val());
        },
        update: function () {
            // Kiem tra ma trang
            ...
            if (QuanLyDichVu_MuonHocBaGoc.ID > 0) {
                //Update
                $.getJSON(this.url + "ad_ctsv_qldv_xacnhan_changestatus.ashx?Type=" + QuanLyDichVu_MuonHocBaGoc.Type + "&&ID=" + QuanLyDichVu_MuonHocBaGoc.ID + "&&STATUS=" + strStatus + "&&PhanHoi=" + strPhanHoi + "&&NgayBatDau=" + strNgayBatDau + "&&NgayKetThuc=" + strNgayKetThuc, function (data) {
                    $('.loader').hide();
                    if (data == 1) {
    ...
    QuanLyDichVu_MuonHocBaGoc.initData();
    QuanLyDichVu_MuonHocBaGoc.bindData();
</script>
<script>
