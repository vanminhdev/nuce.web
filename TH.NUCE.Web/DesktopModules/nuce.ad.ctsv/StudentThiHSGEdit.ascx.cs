...
{
    public partial class StudentThiHSGEdit : PortalModuleBase
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    if (int.TryParse(Request.QueryString["ID"].Trim(), out iID))
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            divSinhVien.InnerHtml = string.Format("Chỉnh sửa thông tin thi học sinh giỏi của sinh viên {0} - {1}", dt.Rows[0]["FulName"].ToString(), dt.Rows[0]["Code"].ToString());
