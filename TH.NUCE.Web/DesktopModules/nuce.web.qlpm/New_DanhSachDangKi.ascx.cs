...
{
    public partial class New_LichSuDangKi : PortalModuleBase
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            #endregion
            divJavascript.InnerHtml = string.Format("<script>var divFilter='{0}';var divContent='{1}'; var divNoiDungHanhDong='{2}';$('#{2}').hide(); var txtTypeAction='{3}';var txtNoiDung='{4}';var txtIDAction='{5}';</script>", divFilter.ClientID, divContent.ClientID, divNoiDungHanhDong.ClientID, txtTypeAction.ClientID,txtNoiDung.ClientID, txtIDAction.ClientID);
        }
            ...
            try
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        DataTable dtTuanHienTai = data.dnn_NuceQLPM_LichPhongMay.getByUpdatedDate(dtNgayBatDau, dtNgayKetThuc, iPhongHocID, -1, 1);
                        if (dtTuanHienTai.Rows.Count > 0)
                        {
                            ...
                            strContent = strContent + string.Format("<td align='center'></td>");
                            ...
                            for (int i = 0; i < dtTuanHienTai.Rows.Count; i++)
                            {
                                ...
                                strContent = strContent + string.Format("<td align='center'><span style='color:blue;cursor:pointer; font-weight: bold;' onclick='duyet({0})'>Duyệt - </span><span style='color:red;cursor:pointer; font-weight: bold;' onclick='huy({0})'>Huỷ</span></td>", dtTuanHienTai.Rows[i]["ID"].ToString());
                                strContent = strContent + string.Format("</tr>");
                            }
                            // Noi dung
        ...
        {
            int ID = int.Parse(txtIDAction.Text);
            string strNoiDung = txtNoiDung.Text;
            string strType = txtTypeAction.Text;
            if(strType.Equals("1"))
            {
                data.dnn_NuceQLPM_LichPhongMay.Approve(ID, strNoiDung, this.UserId, this.UserInfo.DisplayName);
            }
            else
                data.dnn_NuceQLPM_LichPhongMay.Discard(ID, strNoiDung, this.UserId, this.UserInfo.DisplayName);

            Response.Redirect(string.Format("/tabid/{0}/default.aspx", this.TabId));
        }

        protected void btnDuyetAll_Click(object sender, EventArgs e)
        {
