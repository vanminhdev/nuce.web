...
{
    public partial class New_LichPhongMay : PortalModuleBase
    {
        static int iPhong;
        static int iTuan;
        ...
        private void bindData()
        {
            ...
            DataTable dtLichPhongMay = data.dnn_NuceQLPM_LichPhongMay.get(NgayBatDau, NgayKetThuc, -1, 3);
            ...
            strHtmlOut += string.Format("<div style='text-align:center;font-style:italic; font-size:16px;padding-bottom:5px;'>Tuần {0} {4} Ngày {1:dd/MM} đến {2:dd/MM} năm {3:yyyy}</div>", iTuan, NgayBatDau.AddDays(1), NgayKetThuc.AddDays(-1), NgayBatDau,iTuan<23?"":"(tuần thứ "+(iTuan-19).ToString()+" của kỳ 2) ");
            if (iPhong > 0)
            {
        ...
        private string getHtml(DateTime NgayBatDau,int Phong, DataTable dt)
        {
            ...
            for (int i = 2; i < 8; i++)
            {
                ...
                if (drs.Length > 0)
                {
                    ...
                    iTypeLich = int.Parse(drs[0]["Type"].ToString());
                    if (DateTime.Now <NgayBatDau.AddDays(i-1)&& iTypeLich>4)
                ...
                else
                {
                    ...
                    if (DateTime.Now < NgayBatDau.AddDays(i-1))
                    {
