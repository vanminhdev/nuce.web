...
namespace nuce.web.commons
{
    ...
    public class ad_ctsv_studentsquatrinhhoctapgetchitiet : CoreHandlerCommonAdminCTSV
    {
        public override void WriteData(HttpContext context)
        {
           // DataSet ds = data.dnn_NuceCommon_Khoa.getName(-1).DataSet;
            context.Response.ContentType = "text/plain";
            string search = context.Request["ID"].ToString();
            if (search.Equals(""))
            {
            ...
            {
                string sql = string.Format(@"SELECT  * FROM [dbo].[AS_Academy_Student_QuaTrinhHocTap]
              where ID={0}", search);
