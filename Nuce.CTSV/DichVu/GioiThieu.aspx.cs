...
namespace Nuce.CTSV
{
    public partial class GioiThieu : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                string sql = "select top 100 * from AS_Academy_Student_SV_GioiThieu where Deleted<>1 and StudentID=" + m_SinhVien.SinhVienID + " order by LastModifiedTime desc";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            case 1:
                                strContent += string.Format("<td>Kính gửi: {0}</br>Đên gặp: {1} </br>Về việc: {2}</td>", dt.Rows[i]["DonVi"].ToString(), dt.Rows[i]["DenGap"].ToString(), dt.Rows[i]["VeViec"].ToString());
                            ...
                            case 4:
                                strContent += string.Format(@"<td><div>Kính gửi: {6}</br>Đên gặp: {7} </br>Về việc: {8}</div><div style='color:blue;'>* Ngày hẹn: Từ <b>{0}</b> giờ - <b>{1} </b> phút - Ngày <b>{2:dd/MM/yyyy}</b> </br>Đến <b>
                                    {3}</b> giờ - <b>{4} </b> phút - Ngày <b>{5:dd/MM/yyyy}</b></div></td>", dtNgayHenBatDau.Hour, dtNgayHenBatDau.Minute, dtNgayHenBatDau,dtNgayHenDenNgay.Hour, dtNgayHenDenNgay.Minute, dtNgayHenDenNgay, dt.Rows[i]["DonVi"].ToString(), dt.Rows[i]["DenGap"].ToString(), dt.Rows[i]["VeViec"].ToString());
                               ...
                               // strContent += string.Format("<td></td>");
                                break;
                            case 5:
                                strContent += string.Format("<td><div>Kính gửi: {0}</br>Đên gặp: {1} </br>Về việc: {2}<div><div style='color:red;'>- Phản hồi: {3}</div></td>", dtNgayHenDenNgay, dt.Rows[i]["DonVi"].ToString(), dt.Rows[i]["DenGap"].ToString(), dt.Rows[i]["VeViec"].ToString(), dt.Rows[i]["LyDo"].ToString(), strPhanHoi);
