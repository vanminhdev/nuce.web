...
{
    class AsEduSurveyGraduateCauHoiService : IAsEduSurveyGraduateCauHoiService
    {
        private readonly SurveyContext _surveyContext;

        public AsEduSurveyGraduateCauHoiService(SurveyContext surveyContext)
        {
        ...
        public async Task<PaginationModel<QuestionModel>> GetAllActiveStatus(QuestionFilter filter, int skip = 0, int take = 20)
        {
            ...
            var query = _surveyContext.AsEduSurveyGraduateCauHoi.Where(q => q.Status != (int)QuestionStatus.Deleted);
        ...
        {
            var list = await _surveyContext.AsEduSurveyGraduateCauHoi.AsNoTracking()
                .OrderBy(q => q.Order)
        ...
        {
            _surveyContext.ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
            var question = await _surveyContext.AsEduSurveyGraduateCauHoi
                .Where(q => q.Status != (int)QuestionStatus.Deleted && q.Id == Id)
                .FirstOrDefaultAsync();

            var childs = await _surveyContext.AsEduSurveyGraduateCauHoi.Where(o => o.ParentCode == question.Code && o.Status != (int)QuestionStatus.Deleted).ToListAsync();
        ...
        //private void task()
        //{
        //    var questions = _surveyContext.AsEduSurveyGraduateCauHoi.OrderBy(o => o.Id).ToList();
        ...
        //        var q = _surveyContext.AsEduSurveyGraduateCauHoi.Find(answers[i].CauHoiId);
        ...
        {
            var questionCreate = new AsEduSurveyGraduateCauHoi
            {
                Id = Guid.NewGuid(),
                BoCauHoiId = -1,
                DoKhoId = 1,
                Code = $"{_surveyContext.AsEduSurveyGraduateCauHoi.Count() + 1:00000}",
            ...
            _surveyContext.AsEduSurveyGraduateCauHoi.Add(questionCreate);
        ...
        {
            var questionUpdate = _surveyContext.AsEduSurveyGraduateCauHoi
            ...
            {
                //xoá cái cũ
                var oldChilds = await _surveyContext.AsEduSurveyGraduateCauHoi.Where(o => o.ParentCode == questionUpdate.Code && o.Status != (int)QuestionStatus.Deleted).ToListAsync();
                ...
                foreach (var code in question.QuestionChildCodes)
                {
                    var questionChild = await _surveyContext.AsEduSurveyGraduateCauHoi.FirstOrDefaultAsync(o => o.Code == code && o.Status != (int)QuestionStatus.Deleted);
                    if(questionChild != null)
                    {
        ...
        {
            var question = await _surveyContext.AsEduSurveyGraduateCauHoi.FirstOrDefaultAsync(q => q.Id == id);
            if(question == null)
            {
                throw new RecordNotFoundException();
            }
