...
namespace nuce.web.survey.student.Attributes.ActionFilter
{
    ...
    public class AuthorizeActionFilter : ActionFilterAttribute, IActionFilter
    {
        ...
        private void RedirecLoginPage(string role, ActionExecutingContext filterContext)
        {
            ...
            {
                filterContext.Result = new RedirectResult($"/surveyresult/login?loginType=2{(targetUrl2nd == "" ? "" : $"&{targetUrl2nd}")}");
            }
            else if (role == RoleNames.KhaoThi_Survey_Department)
            {
                filterContext.Result = new RedirectResult($"/surveyresult/login?loginType=3{(targetUrl2nd == "" ? "" : $"&{targetUrl2nd}")}");
            }
            else if (role == RoleNames.KhaoThi_Survey_GiangVien)
            {
                filterContext.Result = new RedirectResult($"/surveyresult/login?loginType=4{(targetUrl2nd == "" ? "" : $"&{targetUrl2nd}")}");
            }
            else
            {
                filterContext.Result = new RedirectResult($"/account/login{targetUrl}");
